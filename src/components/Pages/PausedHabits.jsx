...
function PausedHabits() {
  ...
  const pausedHabits = allHabitBasics.filter(
    (habit) => habit.is_tracked === false && habit.is_completed === false
  );
    ...
    <Container>
      ...
      <div>
        ...
        <Table hover size="sm">
          <thead>
            <tr>
              <th className="paused_table_head align-middle">habit name</th>
              <th className="paused_table_head align-middle">start date</th>
              <th className="paused_table_head align-middle">end date</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            {pausedHabits.length != 0 &&
              pausedHabits.map((habit) => {
                return (
                  <tr>
                    <td className="align-middle">{habit.habit_name}</td>
                    <td className="align-middle">
                      {moment(habit.start_date).format("MM/DD/YYYY")}
                    </td>
                    <td className="align-middle">
                      {moment(habit.end_date).format("MM/DD/YYYY")}
                    </td>
                    <td className="align-middle">
                      <OverlayTrigger
                        placement="bottom"
                        overlay={tooltip_unpause}
                      >
                        <Button
                          onClick={() =>
                            handlePause(habit.id, habit.is_tracked)
                          }
                          variant="light"
                        >
